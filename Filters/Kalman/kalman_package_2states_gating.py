...
class IMM2Regimes:
    ...
    def update(self, close_t: float) -> Tuple[float, float, float, float, float]:
        ...
        for j in (0, 1):
            ...
            # eps "gaussien" avant robust
            eps0 = innov / math.sqrt(S0)
            ...
            if self._robust and (not self._robust_high_only or j == 1):
                ...
                if aeps > self._huber_k:
                    # cible: ramener |eps| vers k => S_target = innov^2 / k^2
                    S_target = (innov * innov) / (self._huber_k * self._huber_k)
                    # cap pour éviter des "S" infinis si vraiment énorme
                    S_new = max(S0, min(S_target, S0 * self._robust_S_cap_mult))
                    S = S_new
                    eps0 = innov / math.sqrt(S)
...
def main():
    ...
    outlier_report(eps_mix_test, "eps_mix")
    maskH = (pi_high_test > 0.5)
    maskL = ~maskH
    outlier_report(eps_low_test[maskL],  "eps_low | low")
    outlier_report(eps_high_test[maskH], "eps_high | high")
