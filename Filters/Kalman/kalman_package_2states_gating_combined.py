...
class IMM2Regimes:
    """
    IMM 2 régimes (low/high vol) + robust gating (Huber-like) optionnel.
    - no-leak: utilise uniquement close_t courant
    - robust: down-weight les outliers via inflation temporaire de S (équiv. à augmenter R localement)
    """
    ...
    __slots__ = (
        ...
        "mu", "m", "P",
        "_n", "_jitter",
        "_robust", "_robust_high_only", "_huber_k", "_robust_S_cap_mult",
    )
    ...
    def __init__(
        ...
        *,
        ...
        # évite R=0 (très mauvaise idée en pratique)
    ...
    def update(self, close_t: float) -> Tuple[float, float, float, float, float, float, float, float]:
        ...
        eps_raw = np.empty(2, dtype=float)     # <-- NEW
        eps_rob = np.empty(2, dtype=float)     # <-- NEW
        ...
        for j in (0, 1):
            ...
            eps_raw[j] = innov / math.sqrt(S0)                   # <-- NEW

            # --- Robust gating (Huber-like) ---
            S = S0
            ...
            eps_rob[j] = innov / math.sqrt(S)                    # <-- NEW

            # likelihood et update avec S (robuste)
        ...
        # --- Mix eps (RAW & ROBUST) ---
        eps_raw_low  = float(eps_raw[0])                          # <-- NEW
        eps_raw_high = float(eps_raw[1])                          # <-- NEW
        eps_raw_mix  = float(mu[0] * eps_raw_low + mu[1] * eps_raw_high)  # <-- NEW

        eps_robust_low  = float(eps_rob[0])                       # <-- NEW
        eps_robust_high = float(eps_rob[1])                       # <-- NEW
        eps_robust_mix  = float(mu[0] * eps_robust_low + mu[1] * eps_robust_high)  # <-- NEW
